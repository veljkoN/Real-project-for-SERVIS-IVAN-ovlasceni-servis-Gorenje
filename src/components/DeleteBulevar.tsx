...
const DeleteBulevar:React.FC = () => {
    ...
    const deleteData = () => {
        firebase.firestore().collection('bulevar').where('status','==','Završeno').get().then((doc:DocGT) => {
    ...
    return (
        <div className='container'>
            <Helmet>
                <title>GServis-IVAN | Bulevar - brisanje podataka</title>
            </Helmet>
            <div className='row mt-3'>
                <div className='col-sm-4'></div>
                <div className='col-sm-4'>
                <h5>Brisanje podataka iz magacina Bulevar</h5>
                    <p>Prikaži sve naloge koji su završeni</p>
                    <button className='btn btn-success' onClick={()=>handleClick()}>Prikaži</button>
                </div>
                <div className='col-sm-4'>
                ...
                <div className='table-responsive'>
                    <h4 className='mt-1'>Ukupan broj završenih naloga u magacinu Bulevar {} <span className="badge badge-danger">{data.length}</span>{} </h4>
                    <table className='table table-bordered table-centered table-hover' id='table-to-excel' >
                        ...
                        <tbody>
                            ...
                            <tr key={item.id} style={item.status==='Završeno' ? (styleSU):(undefined) }>
